/**
 * V275/V280 — Estimativa de pedágio via Google Routes API (computeRoutes + TOLLS).
 * V280: só soma legs com tollInfo presente (sem pedágio = tollInfo ausente no leg).
 */
...
function moneyToReais(money) {
  ...
  const nanos = money.nanos != null ? Number(money.nanos) : 0;
...
/**
 * V280 — Soma pedágio apenas de legs com tollInfo (Google: ausência = sem praça no trecho).
 * Ignora estimatedPrice global da rota (evita estimativa fantasma em trajetos urbanos).
 * @returns {number} total em R$ ou 0 se nenhum leg com praça
 */
function extractRouteTollReais(route) {
  ...
  for (const leg of legs) {
    ...
    const legToll = pickBrlEstimatedPrice(tollInfo.estimatedPrice);
    if (legToll != null && legToll > 0) soma += legToll;
  }
  return soma;
}
...
export async function buscarPedagioRoutes(stops, opts = {}) {
  ...
  try {
    ...
    return {
      ok: true,
      valorPedagio,
      formatado: valorPedagio > 0 ? formatDecimal(valorPedagio, 2) : "",
      travelMode,
    };
  } catch (err) {
